...
// 🌟 3. ผูก Event กดที่ตัวลุง Tako หรือข้อความ
mainGifContainer.onclick = () => { if (!isSpinning) startGacha(); };
hintText.onclick = () => { if (!isSpinning) startGacha(); };
...
async function processGachaResult() {
    ...
    try {
        ...
        // 🌟 คำนวณระยะทางไว้ล่วงหน้าเสมอ (เพื่อเอาไปแสดงตอนสรุปผล ไม่ว่าจะเลือกโหมดไหน)
        list = list.map(r => {
...
function displayResult(unseenList, mode) {
    ...
    if (unseenList.length === 0) {
        ...
        // 🌟 ดึงพิกัดไปเปิด Google Maps อย่างแม่นยำ
        ...
        // 🌟 ถ้าคำนวณระยะทางได้ ให้โชว์ป้ายกำกับ
        let distHtml = "";
