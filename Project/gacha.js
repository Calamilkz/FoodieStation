...
function displayResult(unseenList, mode) {
    ...
    if (unseenList.length === 0) {
        ...
        // 🌟 1. ตั้งค่าเริ่มต้น: ค้นหาด้วย ชื่อร้าน + จังหวัด
        let mapUrl = `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(res.name + " " + (res.province || ""))}`;
        
        // 🌟 2. เช็กว่ามีลิงก์ Maps ที่บันทึกไว้หรือไม่ (Priority 1)
        if (res.mapUrl && res.mapUrl.trim() !== "") {
            mapUrl = res.mapUrl;
        } 
        // 🌟 3. ถ้าไม่มีลิงก์ แต่มีพิกัด (Priority 2)
